...
#warning i add
            ...
            UILongPressGestureRecognizer *press = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(photoTap:)];
            press.minimumPressDuration = 1;
            [_photoImageView addGestureRecognizer:press];
...
- (void)photoTap:(UILongPressGestureRecognizer *)sender{
    //解决响应两次的问题
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex != actionSheet.cancelButtonIndex) {
        if (buttonIndex == actionSheet.firstOtherButtonIndex) {
            [_photoBrowser savePhoto]; return;
...
- (void)handleSingleTap:(CGPoint)touchPoint {
    [[NSNotificationCenter defaultCenter ]postNotificationName:@"reloadChat" object:nil];
