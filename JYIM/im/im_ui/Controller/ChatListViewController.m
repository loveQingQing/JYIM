...
@interface ChatListViewController ()<UITableViewDelegate,UITableViewDataSource,IMClientManagerDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!listCell) {
        listCell = [[ChatListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ChatListCell"];
    }
    
    MessageListModel *listModel = self.messagesArray[indexPath.row];
    
    listCell.messageListModel = listModel;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    chatVc.uid = [seletedMessageModel.fromUser isEqualToString:[IMClientManager sharedInstance].uid]?seletedMessageModel.toUser:seletedMessageModel.fromUser;
...
- (void)getMessages
{
  NSMutableArray * messageListArr =  [[IMClientManager sharedInstance].imDB queryMessageList];
 
    for (int i = 0; i < messageListArr.count; i ++) {
        MessageListModel * model = messageListArr[i];
        [model handleMessageText];
        
    }
    [self.messagesArray removeAllObjects];
    [self.messagesArray addObjectsFromArray:messageListArr];
}
