...
-(void)handleShowTimeWithLastMessageModel:(id)lastMessageModel{
    ...
    if (lastMessageModel == nil) {
       ...
       MessageInfoModel * lastModel = (MessageInfoModel*)lastMessageModel;
        NSDate* lastdate = [NSDate dateWithTimeIntervalSince1970:[lastModel.sendTime doubleValue]];
        NSDate* currentDate = [NSDate dateWithTimeIntervalSince1970:[self.sendTime doubleValue]];
        NSTimeInterval timeBetween = [currentDate timeIntervalSinceDate:lastdate];
        
        if (fabs(timeBetween) > showTimeInterval) {
            self.shouldShowTime = YES;
        }else{
            self.shouldShowTime = NO;
        }
    }
}
